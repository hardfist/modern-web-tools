(() => {
  ...
  async function startLoading(files) {
    if (files.length === 1) {
      ...
      if (!match)
        match = /\/\*#\s*sourceMappingURL=data:((?:[^,*]|\*[^/])+),((?:[^ *]|\*[^/])+)(?:[^*]|\*[^/])*\*\//.exec(
          code
        );
  ...
  function createTextArea({
    ...
    function computeScrollbarsAndClampScroll() {
      ...
      let maxScrollX = Math.round(
        longestLineInColumns * columnWidth + textPaddingX * 2 + margin + scrollbarThickness - width
      );
        ...
        if (e.pageX >= x + margin && e.pageX < x + width && e.pageY >= y && e.pageY < y + height) {
          ...
          if (roundedColumn >= 0) {
            ...
            if (row >= 0) {
              ...
              const { index: snappedFlooredIndex, firstMapping, rangeOfMapping } = analyzeLine(
                row,
                flooredColumn,
                fractionalColumn,
                'floor'
      ...
      draw(bodyStyle) {
        ...
        for (let row = firstRow; row <= lastRow; row++) {
          ...
          for (let map = firstMapping; map < mappings.length; map += 6) {
            if (mappings[map + mappingsOffset] !== row || mappings[map + mappingsOffset + 1] >= lastIndex)
              break;
